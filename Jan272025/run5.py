...
# for speed = 550, -2.2
# for backwards, send degrees in negative and backwards = true. speed can be positive value
async def gyro_straight_decel_tank(degrees, angle, correction, speed, backwards = False, log = False, delay=15):
    motor.reset_relative_position(port.F, 0)
    count = 0
    while (True):
        count = count + 1
        ...
        # if less than 50 degrees from target then slow down at decel_speed but keep gyro straight angle
        if(decel_distance > 40):
            if(backwards):
                # apply steering to correct the error
                #print("backwards left velocity : " + str(-(speed-proportion)))
                #print("backwards right velocity: " + str(-(speed+proportion)))
...
async def main():
    ...
    await move_wall_vertical(25, 1000, True)
    await gyro_straight_tank(360, 0, -2.2, 400)
    await gyro_turn_decel(90, "right", 15)
    #straight to shark
    await gyro_straight_decel_tank(630, 90, -2.2, 450)
    await move_wall_horizontal(650, -1000, True)
    runloop.run(gyro_straight_tank(-400, 90, -0.6, 250, True), move_wall_horizontal(250, 1000, True))
    #runloop.run 
    await gyro_turn_decel(100, "right", 25)
    #await gyro_straight_decel_tank(400, 100, -2.2, 450)
    runloop.run(move_wall_horizontal(725, 1000, True), move_wall_vertical(950, 1000, True), gyro_straight_decel_tank(600, 100, -2.2, 450))
    await gyro_turn_decel(90, "left", -15)
    await move_wall_vertical(950, -1000, True)
    await gyro_straight_tank(-250, 90, -0.2, 100, True)
    runloop.run(move_wall_horizontal(325, -1000, True), move_wall_vertical(1000, 1000, True))
    await gyro_turn_decel(70, "left", -15)
    await gyro_straight_tank(400, 70, -1.2, 360)
    #await gyro_turn_decel(87, "right", 15)
    await gyro_straight_tank(700, 100, -2.2, 450)
    await gyro_turn_decel(140, "right", 40)
    await gyro_straight_tank(-500, 140, -2.2, 400, True)
    runloop.run(motor_pair.move_for_degrees(motor_pair.PAIR_1, 750, -2, velocity=720), move_wall_vertical(1025, -1000, True))
