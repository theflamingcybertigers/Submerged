...
async def main():
    ...
    # goto whale
    await gyro_straight_decel_tank(960, 0, -2.2, 450, False, False, 15, 100, 100)
    #print("Before turning for whale: " + str(motion_sensor.tilt_angles()[0]* -0.1))
    await gyro_turn_decel(32, "right", 8)
    #print("after turning for whale: " + str(motion_sensor.tilt_angles()[0]* -0.1))
    await runloop.sleep_ms(30)

    # Go into the whale
    await gyro_straight_tank(150, 32, -0.4, 400)
    await gyro_straight_tank(-120, 32, -0.8, 300, True)
    
    # Turning to the sonar
    await gyro_turn_decel_pivot_on_left(-72, "left" , -45)
    #print("before gyro st next to sonar: " + str(motion_sensor.tilt_angles()[0]* -0.1))
    #await gyro_turn_decel_pivot_on_left(-88, "left" , -45)
    #do sonar
    await gyro_straight_decel_tank(390, -72, -1.8, 400)
    #print("sonar align: " + str(motion_sensor.tilt_angles()[0]* -0.1))
    #await gyro_straight_decel_tank(390, -72, -1.8, 400)
    
    #do sonar whales 
    #await move_wall_vertical(600, 800, True)
    await move_wall_vertical(300, 800, True)
    await runloop.sleep_ms(200)
    await move_wall_vertical(400, 800, True)

    #move to do submarine
    #dont go too right because of gear chain on right, hits wall
    await gyro_turn_decel(-75, "left", -25)

    await gyro_straight_decel_tank(150, -75, -1.4, 450)
    runloop.run(gyro_straight_decel_tank(125, -75, -1.4, 450), move_wall_vertical(500, -1000, True))
    #print("sub align: " + str(motion_sensor.tilt_angles()[0]* -0.1))

    #move wall to submarine and put unknown creature in circle
    await move_wall_horizontal(450, 1000, True)
    await move_wall_vertical(1200, 1000, True)
    runloop.run(move_wall_vertical(1450, -1000, True), move_wall_horizontal(450, -1000, True))
